(function(angular, $) {
    ...
    angular.module('FileManagerApp').controller('FileManagerCtrl', [
		...
		$scope.fileNavigator.eek = function() {
			console.log('eek filenavigaor ctrl');
		}
        ...
        $scope.openEditItem = function() {
            ...
            $scope.apiMiddleware.getContent(item).then(function(data) {
				console.log('got content',data,data.result);
        ...
        $scope.createFolder = function() {
            ...
            var folder=new Item({name:item.tempModel.name,path:item.tempModel.path,fullPath:item.tempModel.fullPath()},item.tempModel.fullPath());
            $scope.apiMiddleware.createFolder(folder).then(function() {
                $scope.fileNavigator.refresh();
		...
		$scope.dropFiles = function(files,folders) {
			console.log(['drop files',files,folders]);
			$scope.apiMiddleware.upload(files, $scope.fileNavigator.currentPath,folders).then(function() {
               console.log(['uploaded',files,folders,'to',$scope.fileNavigator.currentPath]);
                $scope.fileNavigator.refresh();
            }, function(data) {
                var errorMsg = data.result && data.result.error || $translate.instant('error_uploading_files');
                $scope.apiMiddleware.apiHandler.error = errorMsg;
